...
class TestPanel {
  ...
  @InjectMocks PanelAPIimplement panelApi;
   ...
   * ch.ffhs.pa5.escapeconnect.handlers.PanelAPIimplement#getPanes(javax.ws.rs.core.SecurityContext)}.
   */
  @Test
  void testGetPanes() {
    ...
    // This test follows the same steps than GetPanes() which must be mocked
    ...
    topicToQuery.add(
        String.join("/", myDeviceDao.getBasetopic(), myDeviceDao.getDeviceid(), "$state"));
    ...
    // When the method getPanes() calls a DAO, Mockito will mock the return value
    ...
    // Compare if the code above does generate the correct status 200
